...
public class PacketArmorStand {
    ...
    public void spawn(Player player) {
        try {
            ...
            if (headRotation != null) {
                ...
                if (!Utils.MINECRAFT_VERSION_17.atOrAbove())
                    id = id - 1;
                if (!Utils.MINECRAFT_VERSION_15.atOrAbove())
                    id = 12;
                PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                        new Vector3F((float) Math.toDegrees(headRotation.getX()),
                                (float) Math.toDegrees(headRotation.getY()),
                                (float) Math.toDegrees(headRotation.getZ())));
            }
            if (bodyRotation != null) {
                ...
                if (!Utils.MINECRAFT_VERSION_15.atOrAbove())
                    id = 13;
                PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                        new Vector3F((float) Math.toDegrees(bodyRotation.getX()),
                                (float) Math.toDegrees(bodyRotation.getY()),
                                (float) Math.toDegrees(bodyRotation.getZ())));
            }
            if (leftArmRotation != null) {
                ...
                if (!Utils.MINECRAFT_VERSION_15.atOrAbove())
                    id = 14;
                PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                        new Vector3F((float) Math.toDegrees(leftArmRotation.getX()),
                                (float) Math.toDegrees(leftArmRotation.getY()),
                                (float) Math.toDegrees(leftArmRotation.getZ())));
            }
            if (rightArmRotation != null) {
                ...
                if (!Utils.MINECRAFT_VERSION_15.atOrAbove())
                    id = 15;
                PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                        new Vector3F((float) Math.toDegrees(rightArmRotation.getX()),
                                (float) Math.toDegrees(rightArmRotation.getY()),
                                (float) Math.toDegrees(rightArmRotation.getZ())));
            }
            if (leftArmRotation != null) {
                int id = 20;
                if (!Utils.MINECRAFT_VERSION_17.atOrAbove())
                ...
                if (!Utils.MINECRAFT_VERSION_15.atOrAbove())
                    id = 16;
                PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                        new Vector3F((float) Math.toDegrees(leftLegRotation.getX()),
                                (float) Math.toDegrees(leftLegRotation.getY()),
                                (float) Math.toDegrees(leftLegRotation.getZ())));
            }
            if (rightLegRotation != null) {
                ...
                if (!Utils.MINECRAFT_VERSION_15.atOrAbove())
                    id = 17;
                PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                        new Vector3F((float) Math.toDegrees(rightLegRotation.getX()),
                                (float) Math.toDegrees(rightLegRotation.getY()),
                                (float) Math.toDegrees(rightLegRotation.getZ())));
            }

            if (name != null) {
                Optional<?> opt = Optional
                        .of(WrappedChatComponent.fromChatMessage(Utils.colorize(player, this.name))[0].getHandle());
                try {
                    PacketUtils.setMetadata(dataWatcher, 2,
            ...
            if (scale != 1.0) {
                PacketUtils.sendAttributePacket(player, entityId, "generic.scale", scale);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void updateName(Player player) {
        ...
        if (name != null) {
            Optional<?> opt = Optional
                    .of(WrappedChatComponent.fromChatMessage(Utils.colorize(player, this.name))[0].getHandle());
            try {
                PacketUtils.setMetadata(dataWatcher, 2, WrappedDataWatcher.Registry.getChatComponentSerializer(true),
                        opt);
            } catch (Exception e) {
                PacketUtils.setMetadata(dataWatcher, 2, String.class, this.name);
            }
    ...
    public void updateBody(Player player) {
        ...
        if (headRotation != null) {
            ...
            PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                    new Vector3F((float) Math.toDegrees(headRotation.getX()),
                            (float) Math.toDegrees(headRotation.getY()), (float) Math.toDegrees(headRotation.getZ())));
        }
        if (bodyRotation != null) {
            int id = 17;
            if (!Utils.MINECRAFT_VERSION_17.atOrAbove())
            ...
            PacketUtils.setMetadata(dataWatcher, id, Vector3F.getMinecraftClass(),
                    new Vector3F((float) Math.toDegrees(bodyRotation.getX()),
                            (float) Math.toDegrees(bodyRotation.getY()), (float) Math.toDegrees(bodyRotation.getZ())));
        }
        if (leftArmRotation != null) {
            int id = 18;
            if (!Utils.MINECRAFT_VERSION_17.atOrAbove())
    ...
    public PacketArmorStand setScale(double scale) {
        this.scale = scale;
        return this;
    }
