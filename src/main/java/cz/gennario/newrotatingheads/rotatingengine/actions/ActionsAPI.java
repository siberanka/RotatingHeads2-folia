...
public class ActionsAPI {
    ...
    public void loadDefaults() {
        ...
        addAction("console-cmd", (player, identifier, data, replacement) -> {
            if (data.isExist("value")) {
                SchedulerUtils.runTask(Main.getInstance(), () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
                        replacement.replace(player, data.getString("value"))));
        ...
        addAction("broadcast", (player, identifier, data, replacement) -> {
            if (data.isExist("value")) {
                SchedulerUtils.runTask(Main.getInstance(), () -> Bukkit.broadcastMessage(
                        Utils.colorize(player, replacement.replace(player, data.getString("value")))));
            } else if (data.isExist("values")) {
                for (String s : data.getListString("values")) {
                    SchedulerUtils.runTask(Main.getInstance(),
        ...
        addAction("player-cmd", (player, identifier, data, replacement) -> {
            if (data.isExist("value")) {
                SchedulerUtils.runTask(Main.getInstance(), player,
        ...
        addAction("actionbar-all", (player, identifier, data, replacement) -> {
            if (data.isExist("value")) {
                for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
                    ActionBar.sendActionBar(onlinePlayer,
                            Utils.colorize(player, replacement.replace(onlinePlayer, data.getString("value"))));
                }
            } else {
        ...
        addAction("title", (player, identifier, data, replacement) -> {
            if (data.isExist("title") && data.isExist("subtitle")) {
                ...
                Titles.sendTitle(player, fadeIn, stay, fadeOut,
                        Utils.colorize(player, replacement.replace(player, title)),
                        Utils.colorize(player, replacement.replace(player, subtitle)));
            } else {
        ...
        addAction("title-all", (player, identifier, data, replacement) -> {
            if (data.isExist("title") && data.isExist("subtitle")) {
                ...
                for (Player op : Bukkit.getOnlinePlayers()) {
                    Titles.sendTitle(op, fadeIn, stay, fadeOut,
                            Utils.colorize(player, replacement.replace(player, title)),
                            Utils.colorize(player, replacement.replace(player, subtitle)));
                }
            } else {
        ...
        addAction("sound", ((player, identifier, data, replacement1) -> {
            if (data.isExist("value")) {
                float volume = 60;
                float pitch = 20;
                if (data.isExist("volume"))
                    volume = data.getFloat("volume");
                if (data.isExist("pitch"))
        ...
        addAction("gamemode", ((player, identifier, data, replacement1) -> {
            if (data.isExist("value")) {
                SchedulerUtils.runTask(Main.getInstance(), player, () -> player.setGameMode(
                        GameMode.valueOf(replacement.replace(player, data.getString("value")).toUpperCase())));
            }
        }));
        addAction("gamemode-all", ((player, identifier, data, replacement1) -> {
            if (data.isExist("value")) {
                for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
                    SchedulerUtils.runTask(Main.getInstance(), onlinePlayer, () -> onlinePlayer.setGameMode(GameMode
                            .valueOf(replacement.replace(onlinePlayer, data.getString("value")).toUpperCase())));
                }
            }
        }));
        ...
        for (Section section : actionConfigurations) {
            if (section.contains("conditions")) {
                ...
                for (String s : section.getSection("conditions").getRoutesAsStrings(false)) {
                    if (!Main.getInstance().getConditionsAPI().check(player, section.getSection("conditions." + s),
                            replacement))
                        allow = false;
                }
