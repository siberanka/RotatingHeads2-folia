...
public final class Utils {
    ...
    public static String locationToStringCenter(Location location) {
        String loc = location.getWorld().getName() + "(";
        loc += (location.getBlockX() + 0.5) + ",";
        loc += location.getY() + ",";
        loc += (location.getBlockZ() + 0.5) + ")";
        return loc;
    }
    ...
    public static void optiomizeConfiguration(String path) {
        ...
        if (yamlDocument.contains("settings.animations"))
            yamlDocument.remove("settings.animations");
            ...
            switch (stringReader.getMainValue().toLowerCase()) {
                case "rotate":
                    ...
                    if (valueExist(stringReader, "direction")) {
                        Pair<String, String> direction = stringReader
                                .getStringFromData(getData(stringReader, "direction"));
        ...
        if (yamlDocument.contains("head.value")) {
            ...
            if (valueExist(stringReader, "custommodeldata")) {
                Pair<String, Integer> custommodeldata = stringReader
                        .getIntFromData(getData(stringReader, "custommodeldata"));
        ...
        if (yamlDocument.contains("particles"))
            yamlDocument.remove("particles");
        ...
        if (yamlDocument.contains("actions")) {
            ...
            for (String action : actions) {
                ...
                if (valueExist(stringReader, "value")) {
                    yamlDocument.set("actions." + click + ".value",
                            stringReader.getStringFromData(getData(stringReader, "value")).getSecond());
                }
            }
        }
