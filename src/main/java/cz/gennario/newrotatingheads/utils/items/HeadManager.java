 ...
 * HeadSystem [1.1]
 * <p>
 * Util class that solves a bug with gaining a player's head on versions 1.16
 * and higher.
...
public final class HeadManager {
    ...
    public static final CacheType cacheType = CacheType
            .valueOf(Main.getInstance().getConfigFile().getYamlDocument().getString("skull-cache.type"));
    ...
    /**
     * With this method you can get a player's head by nickname or a base64 head by
     * base64 code
     *
     * @param type  Determines whether you want to get the head by name or by base64
     * @param value If you want a player's head, then the player's name. If you want
     *              base64, then base64 code.
    ...
    private static ItemStack getSkullByTexture(String base64) {
        ...
        if (base64.isEmpty() || base64.equals("none"))
            return head;
        ...
        // int version =
        // Integer.parseInt(Bukkit.getServer().getClass().getName().split("\\.")[3].split("_")[1]);

        // TODO: Check for all supported versions
        int version = Integer.parseInt(
                Bukkit.getServer().getBukkitVersion().replaceAll("^(?:\\d+\\.)?(\\d+)(?:\\.[^\\s-]*)?.*$", "$1"));
        if (version >= 18) {
            String skinJson;
            try {
                skinJson = new String(Base64.getDecoder().decode(base64));
            } catch (IllegalArgumentException e) {
                Bukkit.getLogger().log(Level.WARNING, "Invalid Base64 string for head texture: " + base64);
                return head;
            }
    ...
    public static String getPlayerHeadTexture(String username) {
        try {
            String UUIDJson = IOUtils.toString(new URL("https://api.mojang.com/users/profiles/minecraft/" + username),
                    StandardCharsets.UTF_8);
            JsonObject uuidObject = new JsonParser().parse(UUIDJson).getAsJsonObject();
            String dashlessUuid = uuidObject.get("id").getAsString();

            String profileJson = IOUtils.toString(
                    new URL("https://sessionserver.mojang.com/session/minecraft/profile/" + dashlessUuid),
                    StandardCharsets.UTF_8);
