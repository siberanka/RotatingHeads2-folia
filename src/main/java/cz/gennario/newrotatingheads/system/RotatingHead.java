...
public class RotatingHead {
    ...
    public RotatingHead(@Nullable Location location, String name, boolean withConfig) {
        ...
        SchedulerUtils.runTask(Main.getInstance(), () -> Bukkit.getPluginManager().callEvent(loadEvent));
        ...
        if (withConfig) {
            ...
            try {
                ...
                Main.getInstance().getLog().log(Level.WARNING,
                        "Error catched while loading rotatinghead named: " + name + "!");
    ...
    public void loadFromConfig() {
        ...
        this.scale = yamlDocument.getDouble("settings.scale", 1.0);
        ...
        if (yamlDocument.contains("settings.animations")) {
            for (String animation : yamlDocument.getSection("settings.animations").getRoutesAsStrings(false)) {
                ...
                if (animationLoader.animations.containsKey(animation)) {
                    ...
                    HeadAnimationExtender extender = animationData
                            .getExtender(yamlDocument.getSection("settings.animations." + animation));
        ...
        if (yamlDocument.contains("equipment")) {
            for (String slot : yamlDocument.getSection("equipment").getRoutesAsStrings(false)) {
                ...
                HeadEquipmentValue headEquipmentValue = new HeadEquipmentValue(
                        HeadEquipmentValue.HeadEquipmentType.CONFIG);
        ...
        if (yamlDocument.contains("actions")) {
            if (!Cooldowns.isCooldown(player.getUniqueId())) {
                ...
                for (String clickType : yamlDocument.getSection("actions").getRoutesAsStrings(false)) {
                    ...
                    } else if (clickType.toUpperCase().equals(headClickType.name())
                            || clickType.equalsIgnoreCase("ALL")) {
                        Section section = yamlDocument.getSection("actions." + clickType);
                        Main.getInstance().getActionsAPI().useAction(player, section);
                    }
                }
    ...
    public RotatingHead setScale(double scale) {
        this.scale = scale;
        return this;
    }
