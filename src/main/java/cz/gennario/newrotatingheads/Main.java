...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        SchedulerUtils.runTask(this, () -> this.loadHeads(heads));

        new HeadInteraction().register();
        SchedulerUtils.runTaskTimerAsync(this, new HeadRunnable(), 1L, 1L);
    ...
    public void loadLanguage() {
        ...
        languageAPI.setActiveLanguage(configFile.getYamlDocument().getString("language"),
                new ArrayList<>(languageAPI.getLanguages().values()).get(0).getName());
    }
    ...
    public void loadHead(String name) {
        ...
        if (!rotatingHead.getLocation().isWorldLoaded()) {
            log.log(Level.WARNING, "Rotating head " + name
                    + " could not be loaded, because world where the head is supposed to spawn doesn't exist!");
            log.log(Level.WARNING, "To solve this bug, go to the " + getDataFolder() + "/heads/" + name
                    + ".yml and change the location value...");
        } else {
    ...
    public void loadHeadCache() {
        ...
        headCache = new Config(Main.getInstance(), "", yamlDocument.getString("skull-cache.config-settings.name"),
                false);
        try {
            headCache.load();
            ...
            if (yml.contains("values") && yml.contains("expire")) {
                if (new Date().after(TimeUtils.dateFromString(yml.getString("expire")))) {
                    ...
                    instance.add(Calendar.SECOND, TimeUtils
                            .decodeTime(yamlDocument.getString("skull-cache.config-settings.living-time")));
                    yml.set("expire", TimeUtils.dateToString(instance.getTime()));
                }
            } else {
                yml.createSection("values");
                Calendar instance = Calendar.getInstance();
                instance.add(Calendar.SECOND,
                        TimeUtils.decodeTime(yamlDocument.getString("skull-cache.config-settings.living-time")));
                yml.set("expire", TimeUtils.dateToString(instance.getTime()));
            }
            yml.save();
        } catch (IOException e) {
